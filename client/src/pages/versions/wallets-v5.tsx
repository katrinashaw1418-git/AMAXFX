...
function ExchangeRateDisplay({ fromCurrency, toCurrency, amount }: { fromCurrency: string; toCurrency: string; amount: string }) {
  ...
  if (amount && String(amount).trim() !== '') {
    ...
    if (!isNaN(amountNumber) && !isNaN(rateNumber)) {
      ...
      console.log('Calculation:', { amountNumber, rateNumber, grossConverted, transactionFee, netConverted });
      ...
      {amount && (
        <div className="p-3 bg-green-50 border border-green-200 rounded-lg">
          <div className="text-center">
            <div className="text-sm text-gray-600 mb-1">What You'll Receive</div>
            <div className="text-2xl font-bold text-green-600">{convertedAmount}</div>
            <div className="text-sm text-gray-500 mt-1">Sending: {sendingAmount}</div>
          </div>
        </div>
      )}
    </div>
      ...
      <Dialog open={depositModalOpen} onOpenChange={setDepositModalOpen}>
        <DialogContent className="sm:max-w-[450px] max-h-[80vh] overflow-y-auto p-4">
          ...
          <div className="space-y-3">
            ...
            <div>
              <Label htmlFor="deposit-method" className="text-sm">Payment Method</Label>
            ...
            <div className="flex space-x-2 pt-2">
              ...
              <Button variant="outline" className="h-8 text-sm" onClick={() => setDepositModalOpen(false)}>
                Cancel
              </Button>
            </div>
          </div>
      ...
      <Dialog open={withdrawModalOpen} onOpenChange={setWithdrawModalOpen}>
        <DialogContent className="sm:max-w-[400px] max-h-[80vh] overflow-y-auto p-4">
          ...
          <div className="space-y-3">
            ...
            <div>
              <Label htmlFor="withdraw-method" className="text-sm">Withdrawal Method</Label>
