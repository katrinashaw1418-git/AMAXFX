import { apiRequest, apiFetch } from "./queryClient";

export const api = {
  // User
  getCurrentUser: () => apiFetch("/api/user").then(res => res.json()),

  // Portfolio
  getPortfolio: () => apiFetch("/api/portfolio").then(res => res.json()),

  // Wallets
  getWallets: () => apiFetch("/api/wallets").then(res => res.json()),
  ...
  // FX Rates
  getFxRates: () => apiFetch("/api/fx-rates").then(res => res.json()),
  getFxRate: (base: string, target: string) =>
    apiFetch(`/api/fx-rates/${base}/${target}`).then(res => res.json()),

  // AI Recommendations
  getAiRecommendations: () => apiFetch("/api/ai-recommendations").then(res => res.json()),
  ...
  getInvestmentProducts: (filters?: { category?: string; riskProfile?: string; liquidity?: string }) => {
    ...
    return apiFetch(`/api/investment-products${queryString ? `?${queryString}` : ""}`).then(res => res.json());
  },

  // User Investments
  getUserInvestments: () => apiFetch("/api/user-investments").then(res => res.json()),
  getInvestmentProduct: (id: number) =>
    apiFetch(`/api/investment-products/${id}`).then(res => res.json()),
  ...
  getInvestmentPerformance: (params?: { timeframe?: string }) => {
    ...
    return apiFetch(`/api/investment-performance${queryString}`).then(res => res.json());
  },
};
